...
typedef struct __attribute__((__packed__)) superblock {
  uint8_t signature[SIGNATURE SIZE];
...
int set_file_block_alloc(cur_open_t cur_open, int size)
{
  ...
  for (int index = old_count; index < new_count; index++) {
    ...
    if (our_fat[fat_index] == FAT_EOC) {
      cur_open.this_root->size = (i+1) * BLOCK_SIZE;
...
{
  for (int index = (start_index + 1); index < our_super->data_block_count; i++) {
    if (our_fat[i] == 0) {
      return i;
    }
  }
...
int fs_umount(void)
{
  ...
  for (int index = 0; index < our_super->fat_block_count; index++) {
    block_write(1 + i, our_fat + (i * BLOCK_SIZE));
  }
...
int fs_create(const char *filename)
{
  ...
  for (int index = 0; index < FS_FILE_MAX_COUNT; index++) {
    if (strcmp((char*)our_root[index].filename,filename) == 0)
      return -1;
    if (our_root[index].filename[0] == '\0' && empt == -1)
      empty = index;
  }
  ...
  memset(&(our_root[empty_index]),0,BLOCK_SIZE/FS_FILE_MAX_COUNT);
  strcpy((char*)our_root[empty_index].filename,filename);
  our_root[empty_index].start_index = fat_index;
  our_fat[fat_index] = FAT_EOC;
...
int fs_write(int fd, void *buf, size_t count)
{
  ...
  if (block_index != FAT_EOC) {
    remainder = (b_count + output_file.offset) % BLOCK_SIZE;
    our_block_read(block_index, block_buffer);
    memcpy(block_buffer, buffer_copy, remainder);
    our_block_write(block_index, block_buffer);
  }
...
{
  char* block_buffer;
  size_t block_count,block_index,b_count,remainder,b_offset;
  ...
  remainder = read_file.this_root->size - read_file.offset; 
  b_count = (remainder < count) ? remainder : count;     
  block_count = b_count / BLOCK_SIZE + 1;
  ...
  for (int index = 0; index < blk_count; index++) {
    our_block_read(block_index, block_buf + (BLOCK_SIZE*index));
    block_index = our_fat[block_index];
  }
