...
#Create 5 files
for i in $(seq -w 1 4); do echo this_is_test > file${i}; done
echo this_is_file_5 > file5

# add 4 files
#then add 5th file
for i in $(seq -w 1 4); do ./fs_ref.x add disk.fs file${i}; done
...
# remove every file
for i in $(seq -w 1 5); do ./test_fs.x rm disk.fs file${i}; done

# add 5 files
for i in $(seq -w 1 5); do ./test_fs.x add disk.fs file${i}; done
